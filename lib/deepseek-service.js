...
export async function optimizeWithDeepSeek(body, env = process.env) {
  ...
  let optimizationResult = await callDeepSeekWithRepair(
  ...
  if (
    optimizationContext.mode === "html" &&
    !hasSufficientVisibleHtmlEdits(optimizationResult, optimizationContext.editableContent)
  ) {
    optimizationResult = await callDeepSeekWithRepair(
      buildDeepSeekAggressiveRewritePayload(body, deepseekModel, optimizationContext, optimizationResult),
      deepseekApiKey,
      (rawResponseContent) =>
        buildDeepSeekOptimizationRepairPayload({
          body,
...
function buildDeepSeekAggressiveRewritePayload(body, deepseekModel, optimizationContext, previousOptimizationResult) {
  const htmlEditRequirements = buildHtmlEditRequirements(optimizationContext.editableContent);
    ...
    messages: [
      {
        role: "system",
        content:
          "You are revising a webpage for GEO with stronger visible copy changes while preserving the exact DOM structure and overall visual feel. " +
          "Return valid JSON only. Do not wrap your answer in markdown. " +
          "Do not invent unsupported facts. " +
          "This pass must go beyond metadata and make substantial user-facing copy improvements inside existing headings, paragraphs, list items, and CTA links."
      },
      {
        role: "user",
        content: JSON.stringify(
          {
            task:
              "Return JSON with resolvedKeyword, recommendations, and htmlEdits. The previous optimization was too conservative. Rewrite the page copy more aggressively while keeping the same structure and style.",
            requirements: [
              "Always improve title and metaDescription.",
              "Rewrite the H1 if present.",
              "Rewrite multiple visible content blocks, especially the first substantive paragraphs.",
              "Strengthen specificity, topical focus, answerability, and CTA clarity.",
              "Keep the same factual meaning unless the original is vague, in which case make it clearer without inventing facts.",
              "Do not add or remove nodes, classes, ids, styles, scripts, or link destinations."
            ],
            htmlEditRequirements,
            previousOptimizationResult,
            sourcePage: optimizationContext.sourcePage,
            editableContent: optimizationContext.editableContent,
            sourceHtmlSnippet: optimizationContext.sourceHtmlSnippet
          },
          null,
          2
        )
...
function hasSufficientVisibleHtmlEdits(optimizationResult, editableContent) {
  const htmlEdits = optimizationResult?.htmlEdits || {};
  const headings = Array.isArray(editableContent?.headings) ? editableContent.headings : [];
  const paragraphs = Array.isArray(editableContent?.paragraphs) ? editableContent.paragraphs : [];
  const listItems = Array.isArray(editableContent?.listItems) ? editableContent.listItems : [];
  ...
  const changedHeadingCount = countMeaningfulStructuredChanges(headings, htmlEdits.headings);
  const changedParagraphCount = countMeaningfulStructuredChanges(paragraphs, htmlEdits.paragraphs);
  const changedListItemCount = countMeaningfulStructuredChanges(listItems, htmlEdits.listItems);
  const changedLinkCount = countMeaningfulStructuredChanges(links, htmlEdits.links);
  const changedMetadataCount = Number(Boolean(normalizeText(htmlEdits.title))) + Number(Boolean(normalizeText(htmlEdits.metaDescription)));

  const substantiveParagraphs = paragraphs.filter((item) => splitWordCount(item?.text) >= 12);
  const targetParagraphChanges = substantiveParagraphs.length >= 4 ? 3 : substantiveParagraphs.length >= 2 ? 2 : substantiveParagraphs.length >= 1 ? 1 : 0;
  const targetHeadingChanges = headings.length >= 2 ? 2 : headings.length >= 1 ? 1 : 0;
  const targetLinkChanges = links.length ? 1 : 0;

  if (targetParagraphChanges && changedParagraphCount < targetParagraphChanges) {
    return false;
  }

  if (targetHeadingChanges && changedHeadingCount < targetHeadingChanges) {
    return false;
  }

  if (targetLinkChanges && changedLinkCount < targetLinkChanges) {
    return false;
  }

  if (!targetParagraphChanges && !targetHeadingChanges && !targetLinkChanges) {
    return changedMetadataCount >= 1 || changedListItemCount >= 1;
  }

  return changedMetadataCount >= 1 || changedListItemCount >= 1 || changedParagraphCount + changedHeadingCount + changedLinkCount >= 3;
}
...
function countMeaningfulStructuredChanges(originalItems, edits) {
  if (!Array.isArray(originalItems) || !Array.isArray(edits)) {
    return 0;
  }

  let count = 0;
  for (const edit of edits) {
    if (!edit || typeof edit.index !== "number" || typeof edit.text !== "string") {
      continue;
    }

    const original = originalItems.find((item) => item?.index === edit.index);
    if (!original) {
      continue;
    }

    if (hasMeaningfulTextChange(original.text, edit.text)) {
      count += 1;
    }
  }
...
function hasMeaningfulTextChange(originalText, updatedText) {
  const original = normalizeText(originalText);
  const updated = normalizeText(updatedText);
  if (!updated || original === updated) {
    return false;
  }

  const originalWords = splitWordCount(original);
  const updatedWords = splitWordCount(updated);
  const lengthDelta = Math.abs(updated.length - original.length);

  return lengthDelta >= 20 || Math.abs(updatedWords - originalWords) >= 3 || updated.toLowerCase() !== original.toLowerCase();
}
